...
class DeviceAnalysis: 
    ...
    def __init__(self, argv):
        ...
        try:
            opts, args = getopt.getopt(argv,"hiagvmkbz:",["ifile=","analysis=", "gate_v=", "vds=", "model=", "k_points=", "band_route=", "band_zero="])
...
#
    ...
    def create_bulk_calculator (self, basis_set):
        ...
        if (self.voltage_range_input['gv']):
            print "...with Neumann boundary conditions on A/B"
    ...
    def graph_density_of_states (self):
        ...
        outfile = open (self.device_config_path + "_VG_" + str(gv) + "V_VDS_"+ str(vds) + "_zero_" + self.band_zero + "_V_dos.plot", 'w')
        ...
        j = 0
        for i in final_dos:
            if (self.band_zero == 'FermiLevel'):
                energy = energies[j].inUnitsOf(eV)
            elif (self.band_zero == 'Absolute'):

                energy = energies[j].inUnitsOf(eV) + dos_obj.fermiLevel().inUnitsOf(eV)
