...
class DeviceAnalysis: 
    ...
    def usage(self):
        print 'python device_analysis.py --ifile=<inputfile> --model=<huckel|dft> --analysis=<gate_sweep|vds_sweep_nonsc|bulk> (--gate_v=<start,end,steps,vds_value> | --vds=<start,end,steps>)'
        sys.exit()
...
#
    ...
    def vds_sweep_nonsc (self, gate_voltage, vds_voltages):
        ...
        #now remove any gv bias configs for drain_voltage
        gv = gate_voltage.inUnitsOf(Volt)

        #valid_voltages = []
        #for voltage in vds_voltages:
        #    obj_id = "Config, GateV %f, Vds %f" % (gv, voltage.inUnitsOf(Volt))
        #    config = nlread(self.device_config_path, self.config_type[2], 
        ...
        #first try 0 volts VDS
        vds = 0
        trans_objects = nlread(self.device_config_path, TransmissionSpectrum)
        ...
        for voltage in vds_voltages:
            current = trans_object.current(
                      electrode_voltages=(0.5*voltage, -0.5*voltage)
            )
...
if __name__ == "__main__":
        ...
        if analysis_type == "bulk":
        ...
        elif analysis_type == "vds_sweep_nonsc":
            pdb.set_trace()
            vds_voltage_list = np.linspace(dev_anal_obj.vds_start, dev_anal_obj.vds_end, num = dev_anal_obj.vds_points, endpoint=True) * Volt

            print "Sweeping VDS voltages non-selfconsistently: ", vds_voltage_list
            ...
            dev_anal_obj.vds_sweep_nonsc (gate_voltage, vds_voltage_list)
